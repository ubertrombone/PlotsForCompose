...
class XAxisLineModelImpl(initialState: AxisLineStates.XState): InstanceKeeper.Instance, XAxisLineModel {
    override val scope = CoroutineScope(Dispatchers.IO + SupervisorJob())
    override val axisLineState: MutableValue<AxisLineStates.XState> = MutableValue(initialState)
    ...
    override fun updateAxisPosition(position: AxisPosition.XAxis?) {
        scope.launch {
    ...
    override fun resetAxisLine() {
        scope.launch {
            axisLineState.update { AxisLineStates.XState() }
        }
    }
