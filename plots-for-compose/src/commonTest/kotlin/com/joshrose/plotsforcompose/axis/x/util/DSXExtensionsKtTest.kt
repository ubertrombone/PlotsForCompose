...
class DSXExtensionsKtTest {
    ...
    private val config = ContinuousLabelsConfigDefaults.continuousLabelsConfigDefaults().copy(
        yOffset = random.nextInt(0, 50).dp,
    ...
    fun testLabelYPosition() {
        XAxisPosition.values().forEach { xPos ->
            ...
            val offsetY =
                if (xPos == TOP) y.minus(labelSize.height.div(2f)).minus(config.yOffset.value)
                else y.plus(labelSize.height.div(2f)).plus(config.yOffset.value)
            ...
            assertEquals(config.yOffset.value, abs(y.minus(offsetPoint)))
        }
    }
